...
Strictly, CommonMark takes a destination as written and does not require decoding — which is exactly why this is proposed as a user option rather than unconditional behaviour.

### Why it matters

Percent-encoding local paths is what several Markdown tools emit, so files written elsewhere and read in Emacs are affected. Obsidian is the clearest case: it percent-encodes and does not use pointy brackets at all.
...
So the two spellings are not interchangeable in practice: a tool emits one or the other, and Obsidian emits this one. Note that combining them (`<my%20file.md>`) is pointless, since brackets already permit a literal space — and indeed it never occurs in that corpus. The reproducer covers it only for completeness.

### Recipe

`destination-repro.el` is attached; it stubs `find-file` and `browse-url`, so nothing is opened or browsed. With the bracket patch applied but not this one:
...
**The default.** `t` is proposed because the reported symptom is "my link does not open my file", and a fix that is off by default does not fix it; the `%XX` guard keeps plain paths untouched. If you would rather default to `nil` for strict conformance, the patch works either way — only the `defcustom` line changes.
...
That is inherent, not a flaw in the patch, and it is the reason for the option: anyone with such names sets it to `nil` and keeps correct behaviour for them. It is also why this is a separate report from the bracket fix, which has no such corner.
