...
4.  **RTL Margin Rendering:** On Right-to-Left (R2L) rows (e.g., Hebrew or Arabic text), the left margin area is rendered with the default face (black foreground, white background) regardless of any overlays or `margin` face customization. This appears to be a limitation in how the display engine handles `LEFT_MARGIN_AREA` for reversed glyph rows. (Demonstrated by **`face-margin-test-008`**).
