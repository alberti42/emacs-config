      ...
      (let ((trailing-end (point)))
        ...
        (let ((trailing-start (point)))
          (cond ((and (< trailing-start trailing-end)
                 ...
                 ;; These shenanigans are to identify optional
                 ;; trailing sequential octothorpes, fontify them as
                 ;; the delimiters they are, and remove whitespace
                 ;; between heading text and the octothorpes.  The
                 ;; grammar is silent on these despite CommonMark.
...
NODE can be a list, list_item, or list_marker_."
  (let ((type (treesit-node-type node)))
    (cond ((equal "list_item" type)
           (setq node (treesit-node-parent node)))
          ((string-prefix-p "list_marker_" type)
           (setq node (treesit-node-parent
                       (treesit-node-parent node)))))
    (let ((depth 0))
      (while (not (equal (treesit-node-type node) "section"))
        (setq node (treesit-node-parent node))
