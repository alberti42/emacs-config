...
;;
;; LOCAL COPY — upstream (https://github.com/justbur/emacs-vdiff-magit) is
;; unmaintained (last commit 2022).  This copy patches two API breakages
;; introduced by Magit refactors:
;;
;; 1. `magit-get-revision-buffer' was removed and replaced by the internal
;;    `magit--get-blob-buffer', which has the same signature (REV FILE).
;;
;; 2. `magit-find-file-index-noselect' dropped its second argument; all call
;;    sites that passed `t' as the second arg have been updated to omit it.
;;
;; 3. In `vdiff-magit-stage', the HEAD buffer (buf-a) is now explicitly marked
;;    read-only — editing it is meaningless as it is discarded on exit.
;;
;; TODO: file a PR upstream if the project shows signs of life again.
...
FILE has to be relative to the top directory of the repository."
  ...
  (magit-with-toplevel
    (let* ((buf-a (with-current-buffer (or (magit--get-blob-buffer "HEAD" file)
...
(defun vdiff-magit--kill-buffer-if-temporary (buffer)
  "Kill BUFFER if it does not point to a file on disk."
...
Expected to be used as a callback to `vdiff-buffers'."
...
FILE must be relative to the top directory of the repository."
  (interactive
   (list (magit-read-file-choice "Show staged changes for file"
                                 ...
                                 "No staged files")))
  (vdiff-buffers
   (or (magit--get-blob-buffer "HEAD" file)
       (magit-find-file-noselect "HEAD" file))
...
stash that were staged."
  ...
  (-let* ((rev-a (concat stash "^1"))
    ...
    (if (and vdiff-magit-show-stash-with-index
        ...
        (let ((buf-a (magit--get-blob-buffer rev-a file-a))
              (buf-b (magit--get-blob-buffer rev-b file-b))
              (buf-c (magit--get-blob-buffer rev-c file-c)))
          (vdiff-buffers3
