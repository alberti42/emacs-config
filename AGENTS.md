...
- `org-config.el`: Org mode plus inline LaTeX previewing and Python babel, on **built-in Org** (`:straight (org :type built-in)` — not a bare `:straight nil`, since `org-appear` requires `org` and would otherwise rebuild the leftover fork checkout onto `load-path`). Migrated off **tecosaur's (now unmaintained) fork** — the fork-only karthink live `org-latex-preview` never landed upstream, so math rendering here is the classic on-demand `org-latex-preview` (`C-c C-x C-l`, dvisvgm SVG via `org-preview-latex-default-process`, sizing via `org-format-latex-options :scale`). Two hooks restore fork-like ergonomics the classic pipeline lacks: previews are recoloured on OS light/dark theme switch (`enable-theme-functions`, re-runs LaTeX) and rescaled on buffer text zoom (`text-scale-mode-hook`) by adjusting the SVG overlay image's `:scale` in place — no LaTeX re-render, since regenerating via `org-format-latex-options :scale` invalidates the fragment cache and hangs Emacs for seconds per zoom step. Baseline size is `org-config-latex-preview-base-scale`. The full fork-based config is preserved but unloaded in `org-karthink-config.el`. Installs `org-appear` for emphasis-marker auto-toggling. Tunes `C-c '` (`org-edit-special`) with `org-src-*` settings that `lsp-python-config.el` depends on. Python babel defaults are minimal (`:results output :exports both`) — matplotlib setup belongs in per-file blocks or yasnippets. → `docs/modules/org-config.md`
