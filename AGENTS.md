...
- `vulpea-config.el`: note database over the org notes migrated from the Obsidian work vault — **vulpea v2**, which is standalone (v2 dropped org-roam and owns its own SQLite db). Also the single owner of the settings that key off `:ID:`, because *three* systems share that one property: `org-id` (`id:` links), `org-attach` (attachment directory) and vulpea (database primary key). Each note's `:ID:` is the `uuid` its Obsidian front matter carried, so links, external UUID references and attachment directories survive renames. `vulpea-config-notes-directory` is the one place to change if the tree moves; state (db, `org-id` locations) lives under `$XDG_CACHE_HOME/emacs/`, never in the notes tree or this worktree. `C-c n f`/`i`/`b` for find/insert/backlink. Two gotchas: **vulpea does not hook `org-id`** — its own commands resolve through the db, but a plain `[[id:…]]` link goes through `org-id-locations`, which nothing populates automatically, hence `M-x vulpea-config-update-id-locations` after a conversion; and `org-attach-id-dir` must match the converter's `--attach-dir`, since a mismatch yields an *empty* attachment directory rather than an error. Sets `org-id-uuid-program` to `"uuidgen | tr A-Z a-z"` because macOS `uuidgen` prints uppercase, `org-id` shells out to it verbatim, and ID lookup is case-sensitive. Autosync is guarded on the tree existing, so a not-yet-run conversion degrades to "installed but idle" rather than erroring at startup. The tree itself is produced by `etc/goodies/obsidian-to-org.py` (see its header for the conversion invariants).
- `org-attach-crossref.el`: teaches the stock `attachment:` link type a second form, for referring to **another note's** attachment. Stock `attachment:` carries only a filename, which `org-attach-expand` resolves against the *current* node, so there is no syntax for a cross-reference; this adds `[[attachment:<uuid>/file]]`, discriminated by a leading UUID and resolved via `org-attach-dir-from-id`. Because such a link names its target only by ID, it survives renaming *and* moving that note. Implemented as advice on `org-attach-expand` — the one choke point shared by following (`org-attach-follow`), inline preview (`org-attach-preview-file`) and export (`org-attach-expand-links`), so all three learn the form together. Loaded from `vulpea-config.el`. The UUID pattern is deliberately not version-specific (the vault carries both v4 and v5 IDs), and a filename that merely *contains* a UUID stays local, since the discriminator is a UUID followed by `/` as the leading path component.
