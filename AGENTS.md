...
- `magit-config.el`: Magit Git porcelain + Forge (GitHub/GitLab) integration. Includes `vdiff` for side-by-side diffs. Disables line numbers in `git-commit-mode` and `git-rebase-mode` buffers.
