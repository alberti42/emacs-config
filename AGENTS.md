...
- `pyenv-config.el`: per-buffer pyenv version selection, with no external package dependency. Relies on pyenv's own shim behavior as the default: shims walk up from the subprocess CWD looking for `.python-version`, and Emacs subprocesses inherit `default-directory` as CWD, so opening a file inside a project is enough for `python`/`pip`/etc. to resolve correctly — no Emacs-side logic required. On top of that default, the module adds two override paths that both set `PYENV_VERSION` inside a buffer-local `process-environment`: (1) a dir-local variable `pyenv-version` (string, declared safe via `safe-local-variable`) applied from a `hack-local-variables-hook` — useful for project overrides you don't want to commit as a `.python-version` file; (2) `pyenv-activate-buffer` (interactive; completing-read over `pyenv virtualenvs --bare` via `process-lines` — same list `pyenv activate <TAB>` offers, i.e. virtualenv canonical names and aliases) for ad-hoc picks. `pyenv-deactivate-buffer` tears the buffer-local env back down. Public names use the `pyenv-` prefix (matching the underlying tool); the filename keeps the `-config.el` convention of the rest of the repo. The `pyenv-mode` package is intentionally NOT used — it would duplicate work already done by `env-config.el` (shims in `exec-path`) and would force global rather than per-buffer scope.
