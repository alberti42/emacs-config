...
- `vulpea-config.el`: note database over the org notes migrated from the Obsidian work vault — **vulpea v2**, which is standalone (v2 dropped org-roam and owns its own SQLite db). Also the single owner of the settings that key off `:ID:`, because *three* systems share that one property: `org-id` (`id:` links), `org-attach` (attachment directory) and vulpea (database primary key). Each note's `:ID:` is the `uuid` its Obsidian front matter carried, so links, external UUID references and attachment directories survive renames. **No vault directory is named anywhere in this configuration** — there is no vault list and no startup setting. A vault becomes known by being opened, and `vulpea-vault-history` (savehist) is the whole of what Emacs knows about which vaults exist; `vulpea-config--initial-vault` resumes its most recent entry at startup, walking down past any vault whose directory is currently missing. Reading the history that early works because savehist restores it by plain `setq` when `savehist-mode` starts (init.el:299), long before this file is loaded (init.el:636) and before `switch.el`'s own `defvar`, which leaves an already-bound value alone — hence `bound-and-true-p` at the call site. The vault actually resumed is then pushed to the front of the history at the foot of this file, so that skipping an unreachable one is recorded as what it is. **`vulpea-config-notes-directory` nil is a supported state, not a broken one** — a first run with an empty history opens no vault, autosync stays off with a message, and the first note opened from a vault activates it via the `find-file-hook` guard (that branch already existed). Every consumer was made nil-safe accordingly: `vulpea-vault--candidates` (drops the "current" entry), `vulpea-vault-switch` (nothing to close; treats "no vault was open" as "the watcher should be on", since an absent watcher then says nothing about intent), `vulpea-vault-special-directory`, `vulpea-vault--context-directory` — while the commands that are meaningless without a vault (`vulpea-vault-orphans`, `vulpea-config-update-id-locations`, note creation with no daily folder) go through `vulpea-config-vault-or-error` so nil fails as one clean `user-error` instead of a wrong-type-argument deep inside. `vulpea-config-apply-vault` derives the other five settings from a root (state dir, attach dir, `org-attach-id-dir`, `vulpea-db-sync-directories`, `vulpea-db-location`) and is called both at load and by `M-x vulpea-vault-switch` (`vulpea-vault/switch.el`) — whose prompt offers `vulpea-vault-history` (the vaults actually opened, most recent first, persisted via `savehist-additional-variables` and appended to it with `with-eval-after-load 'savehist` so the order of loading does not matter), the active vault last, and project.el's `... (choose a directory)` escape at the end for a vault reached by path; a remembered vault whose directory is missing is left out of the prompt but kept in the history, and the history entry is added only after the switch succeeds. The command closes the leaving vault's buffers (notes and dired, `save-some-buffers` first), stops the watcher, closes the database, re-points, and restarts — a live vault change, no Emacs restart. Closing the buffers is required, not tidy: `org-attach-id-dir` is a single global with no per-buffer form, so a note left open from the old vault would resolve `attachment:` links against the new vault's store and find nothing (dir-locals, being per-buffer, stay correct). Buffers are closed *before* anything is re-pointed, so declining to kill a modified note leaves the vault unchanged rather than half-switched. The same module guards the other direction from `find-file-hook`: opening an org file whose vault is not the active one offers switch / open anyway / cancel, since that is exactly the case where `attachment:` links fail silently. A directory counts as a vault **only** because its `.dir-locals.el` declares `(vulpea-vault-version . N)` — the canonical marker; the mere presence of a `.dir-locals.el` means nothing, most projects have one (this repo included). `vulpea-vault-schema-version` in `scheme.el` is what these modules implement; a vault declaring anything else, or nothing, is still opened but `vulpea-vault-version-check` warns — once per vault per session, since the check runs for every note opened there. `org-id` locations live under `$XDG_CACHE_HOME/emacs/` (they span every org file, not one vault); the database lives in the vault's own `.vulpea/`. `C-c n f`/`i`/`b` for find/insert/backlink. Two gotchas: **vulpea does not hook `org-id`** — its own commands resolve through the db, but a plain `[[id:…]]` link goes through `org-id-locations`, which nothing populates automatically, hence `M-x vulpea-config-update-id-locations` after a conversion; and `org-attach-id-dir` must match the converter's `--attach-dir`, since a mismatch yields an *empty* attachment directory rather than an error (likewise `vulpea-config-notes-directory` vs its `DEFAULT_OUT`). IDs are minted by an `:override` on `org-id-new` returning `(uuid-to-string (uuid-v4))`: Org 9.8.7 still forks `org-id-uuid-program`, which is uppercase on macOS, and ID lookup is case-sensitive. Also extends the stock `attachment:` link type with an `attachment:<uuid>/file` form for **another note's** attachment (stock `attachment:` carries only a filename, resolved against the *current* node, so there is no cross-reference syntax); implemented as advice on `org-attach-expand`, the one choke point shared by following, inline preview and export. Autosync is guarded on the tree existing, so a not-yet-run conversion degrades to "installed but idle" rather than erroring at startup. The tree itself is produced by `etc/goodies/obsidian-to-org.py` (see its header for the conversion invariants). Vault-specific behaviour lives in `vulpea-vault/`, one concern per file, loaded from here the way `completion.el` loads `completions/`: `modified-keyword.el` (refreshes `#+modified:` on save, only in notes that already have it), `bibdesk.el` / `pdffile.el` / `message.el` (the `x-bdsk:`, `pdffile:` and `message:` link types), `attachments.el` (`M-x vulpea-vault-orphans`, the dangling-link and unreferenced-attachment report), `scheme.el` (what makes a directory a vault at all: `vulpea-vault-version`, declared in its `.dir-locals.el`, versus `vulpea-vault-schema-version` here; the version governs the whole vault↔config contract — folder roles, tag vocabulary, templates — which is why it lives on its own rather than inside any one of them), `directories.el` (`vulpea-vault-special-directories`, an alist of role → folder — currently just `daily` — read back by `vulpea-vault-special-directory`, which resolves against the vault root because the caller is usually outside the vault; keeps folder names like `01 Daily notes/` out of this repo), `tags.el` (lets a note tree declare its own tag vocabulary from its `.dir-locals.el`: marks `org-tag-alist` and `org-tag-persistent-alist` safe as file-locals behind a predicate admitting only tag names, selection characters and grouping keywords, then re-runs `(org-set-regexps-and-options 'tags-only)` from `hack-local-variables-hook`. **That recompute is not optional** — dir-locals are applied after the major mode has run, and `org-mode` has already derived the buffer-local `org-current-tag-alist` (the value every consumer reads) from the globals, so a dir-local `org-tag-alist` alone changes nothing. The vault's tags live in `~/org/Work/.dir-locals.el`; Obsidian's nested tags appear there as org tag *groups*, the converter having kept the last segment of `#Teaching/E4`. The converter still emits `00 Meta/org-tag-alist.el` for a fresh vault, but this vault's copy was folded into the dir-locals and deleted), and `create.el` (`vulpea-create-default-function` — a new note goes in the current buffer's `default-directory` when that is inside the vault (dired's listed directory, the directory of the note being read, a shell's cwd), else becomes a note under `<daily>/<year>/`, where `<daily>` is the folder the vault gives the `daily` role (vault root if it declares none); that tree is the one exception to following `default-directory`, since its subdirectory is the note's year rather than a topic, so creating from a 2024 daily note still files under the new note's own year; it is named after its title, opens with today's date, and is seeded with `#+created:`/`#+modified:` plus the folder's `vulpea-vault-template` — tags, extra keywords, a body. That variable is declared **per folder in the vault's own `.dir-locals.el`**, under the directory keys dir-locals already supports, so Emacs resolves the folder and nothing here matches paths; it is the Obsidian Templater `folder_templates` configuration carried over, reconciled with what the converted notes actually carry. A deeper directory key *replaces* a shallower one's value rather than merging, so each entry is self-contained and absent keys fall back to code. `vulpea-vault-template-p` admits inert data only — no function symbols — so the vault cannot introduce code; note that **one unsafe variable makes Emacs discard the entire `.dir-locals.el`**, not just that variable, which is why `tags.el` must be loaded for the templates to apply at all).
