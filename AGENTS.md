...
- `emacs-config-core.el`
  ...
  - Defines the two places this config's machine-local files go — never the git worktree `user-emacs-directory` points at — and the distinction between them is the thing to get right when adding a setting: `emacs-config-cache-dir` / `emacs-config-cache-file` (`$XDG_CACHE_HOME/emacs/`) for **derived** data, where something else is the source of truth and deleting the tree costs only rebuild time (`org-id-locations-file` rescanned from the org files, `latex-to-svg`'s rendered SVGs recompiled from their LaTeX); `emacs-config-state-dir` / `emacs-config-state-file` (`$XDG_STATE_HOME/emacs/`) for **state**, i.e. what the user's own past actions produced with nothing to rebuild it from (`recentf-save-file`, `project-list-file`, `treemacs-persist-file`, `safe-locals-file`, `osx-dictionary-last-dictionary-file`) — losing it is no drama and not worth git, but no recomputation brings it back. Both helpers create their directory. `emacs-config-state-file` also takes an optional LEGACY path (or list of them) and renames an existing file into place the first time, since a state file that moves must travel with its setting or the user silently loses it.
...
- `mode-line-config.el`: global content policy for the stock mode line. Hides uninformative minor-mode lighters (`ARev`, `LTeX+`, `GitGutter`, `WK`, `ElDoc`, and visual-line's plain `Wrap`) without disabling their modes; keeps informative lighters such as `SMerge`, `Ref`, soft-wrap's width, active LSP clients, Flycheck counts, and `yas`. Reapplies the lighter policy through `after-load-functions`, so packages loaded lazily receive it too.
...
- `recentf-config.el`: recently visited files list, persisted under `emacs-config-state-dir`.
- `safe-locals-config.el`: the list of directories whose `.dir-locals.el` is trusted wholesale (`safe-local-variable-directories`), kept in `safe-directories.eld` under `emacs-config-state-dir` rather than in this repo — it is accumulated one approval at a time on one machine. Loaded from `init.el` immediately after the bootstrap (it needs `emacs-config-state-file`) and before anything visits a file. The file exists because Emacs's own way of growing that list is a dead end here: the `+` answer at the unsafe-local-variables prompt calls `customize-push-and-save`, which writes to the `custom-file` `emacs-config-core.el` deliberately never loads, so every `+` is forgotten at the next restart — `M-x safe-locals-add-directory` is the replacement. Entries may use `~`; they are expanded and given a trailing slash on load, matching the documented contract of `safe-local-variable-directories` (Emacs currently compares with `file-equal-p`, which would accept an abbreviated name anyway). A malformed file warns and is treated as empty rather than aborting startup.
...
- `project-config.el`: project.el settings — sets `project-vc-merge-submodules nil` so git submodules are treated as independent project roots rather than merged into the parent repo, and moves `project-list-file` (which defaults into `user-emacs-directory`, a symlink into the dotfiles worktree here) to `emacs-config-state-dir`.
...
- `avy-config.el`: label-based jumping to any visible text (tmux-thumbs / Vimium gesture) via `avy`. Bound inside the built-in **goto-map** alongside `M-g g`/`M-g c`/`M-g n`: `M-g a` = `avy-goto-char-timer` (type as many chars as you like, end with `RET`, labels appear on what is left) as the primary gesture, `M-g A` = `avy-goto-word-0` (labels every word in every visible window at once, no search step — the literal tmux-thumbs gesture, but it exhausts the one-character labels on a dense screen), `M-g l` = `avy-goto-line`. **`avy-timeout-seconds` is nil**, which removes the inter-keystroke deadline rather than tuning it: the variable is used in exactly one place, as the `SECONDS` argument of `read-char` in `avy--read-candidates`, and `read-char` with `SECONDS` nil just waits — so the pattern is ended by `RET` (`avy-enter-times-out`, default t) at the user's pace, and `avy-single-candidate-jump` then fires immediately instead of after a pause. No advice, nothing patched; an earlier `:around` on `read-char` that injected `RET` when the prompt reported one candidate was dropped once this was found. A single chord such as `M-j` was deliberately *not* used — `M-j` (`default-indent-new-line`) is in daily use here as a `C-j` substitute — and `M-g` survives a TTY frame just as well, unlike Super, `C-;` or `C-.`. Labels are home-row only (`avy-keys`), drawn `at-full` over the target rather than inserted, so alignment-sensitive buffers (org/markdown tables) do not reflow. avy's dispatch keys (`n` copy, `y` yank, `t` teleport, `x`/`X` kill, `m` mark, …) are left at their defaults: pressing one *before* a label acts on the target without moving point, which is the "hint it into the clipboard" half of tmux-thumbs. Considered and rejected: `flash` (Prgebish/flash) — it is search-first (type a pattern, *then* labels appear) rather than label-first, which is a different gesture from the one this binding is for.
...
- `lsp-core.el`: shared LSP infrastructure used by every `lsp-*-config.el`. Installs `lsp-mode`, `flycheck` (left-fringe indicators and current-buffer mode-line counts; lsp-mode's duplicate workspace diagnostic counter is disabled here), and force-requires `lsp-diagnostics` so its faces are defined before any server uses them. Sideline rendering via the `sideline` framework — `sideline-flycheck` for diagnostics, `sideline-lsp` for code actions (replaces `lsp-ui-sideline`; `lsp-ui` is no longer used). Hands completion off to corfu+cape (`lsp-completion-provider :none`). Depends on `yasnippet-config.el` being loaded first. `C-c l` keymap prefix. → `docs/modules/lsp-core.md`
...
- `org-config.el`: Org mode plus inline LaTeX previewing and Python babel, on **Org from its own repository**, not the copy bundled with Emacs — that one moves only when the Org maintainer commits a released tree into emacs.git as `Update to Org 9.8.N`, so it trails the 9.8.x line by weeks. The recipe is straight's own for Org, pointed at Savannah itself (`:host nil`, `:repo "https://git.savannah.gnu.org/git/emacs/org-mode.git"`) rather than at the `emacs-straight/org-mode` GitHub mirror, which can lag it by days, with `:branch "main"`, the 10.0 development line where features land — the `bugfix` branch carries fixes for the released 9.8.x line only, so a change merged into main reaches it no earlier than the 10.0 release, and main is pre-release in exchange. It is written out in full because a use-package recipe is literal data: `:local-repo "org"` because `straight-recipes-org-elpa--build` looks for the tree at `straight/repos/org/lisp`, `:depth full` because that step runs `git describe --match release*` to write `org-version.el`, and `:build (:not autoloads)` because it writes `org-loaddefs.el` itself. Registering the recipe here is also what pins `org-appear`, which declares `(org "9.3")` and would otherwise make straight rebuild whatever checkout sits in `straight/repos/org`; and nothing may load `org` before this module runs, or the bundled copy wins. Migrated off **tecosaur's (now unmaintained) fork**. In-buffer math is rendered by the homegrown **`org-latex-to-svg`** package (a front-end over the standalone **`latex-to-svg`** engine, both in `~/Documents/Programming/Emacs/`; `org-mode-hook` enables `org-latex-to-svg-mode`). The shared `latex-to-svg` engine is registered and configured centrally in **`latex-to-svg-config.el`** (loaded first in init.el), not here — this file only registers the `org-latex-to-svg` front-end and turns the mode on. The engine compiles each equation once (content-addressed), color- and size-independent, so previews **recolour on OS theme switch and rescale on text zoom straight from cache with no LaTeX recompile** — the fork's ergonomics without the fork. Built-in Org's classic `org-latex-preview` (dvisvgm) stays as a fallback when the mode is off. The earlier stopgap (classic preview + `org-format-latex-options` recolour/rescale hooks) and `org-startup-with-latex-preview` were removed. The full fork-based config is preserved but unloaded in `org-karthink-config.el`. Also sets `org-id-locations-file` (via `emacs-config-cache-file`, so `org-id`'s ID→file map stays out of the worktree): it spans every org file this Emacs knows, so it belongs with org rather than with any one vulpea vault — keeping it in step with vulpea's db is vulpea's own doing — it registers what it indexes, drops what it forgets, and registers a whole tree when autosync starts. Installs `org-appear` for emphasis-marker auto-toggling. Enables `org-num-mode` from `org-mode-hook`, so section numbers are drawn as overlays instead of living in the heading text — the notes imported from Obsidian carried them literally (`5.4.2. TensorFlow with TensorRT support`), which put the number inside every `[[*Heading]]` link target and broke every link below an inserted section. Tunes `C-c '` (`org-edit-special`) with `org-src-*` settings that `lsp-python-config.el` depends on. Python babel defaults are minimal (`:results output :exports both`) — matplotlib setup belongs in per-file blocks or yasnippets. → `docs/modules/org-config.md`
...
- `markdown-config.el`: Markdown reading and authoring. Configures `markdown-ts-mode` (tree-sitter backed, bundled with Emacs 31) directly via `:mode` on `\\.md\\'` and `\\.markdown\\'`; the legacy `markdown-mode` package was removed once ts-mode reached parity for note-taking workflows. Everything in the file is CommonMark, and it carries only what the bundled mode still does not do — the mode now hides inline-link markup itself and makes every inline link, reference link, autolink and bare URL a real text button (`markdown-ts--fontify-link-node`), so `RET`/`mouse-1` follow links with nothing added here and no follow-link chord is bound. What remains: two `:around` advices, one making `markdown-ts--fontify-image` accept the pointy-bracket (`![a](<path with spaces>)`) and `%20`-encoded destinations its bare `expand-file-name` rejects, the other rerouting `markdown-ts--make-link-button`'s stock `find-file` action for schemeless URLs through `markdown-config--follow-local-link`, so a Markdown target opens with `find-file`, a non-Markdown local file opens `dired` with the cursor on it, a missing file signals `user-error` with the resolved path, and full URLs go to the browser. Inline links inside table cells get their own regex font-lock keyword, because the grammar's range rule embeds `markdown-inline` in `(inline)` nodes only and a `pipe_table_cell` is not one, so no `inline_link` node exists there; the surrounding markup is blanked with a width-preserving `(space :width N)` `display` rather than `invisible`, which would collapse it and misalign the table. Those table links (and any syntax an optional module registers) carry `markdown-config--link-keymap`, giving them the same `RET`/`mouse-1`/`mouse-2` gestures as an upstream button via `markdown-config-follow-link-at-point`; non-CommonMark syntaxes plug into that dispatcher through the `markdown-config-follow-link-functions` abnormal hook rather than by adding a branch. Code-fence lines are collapsed while editing with an overlay `display` of `""` plus `reveal-mode` (upstream hides whole fence lines in `markdown-ts-view-mode` only, leaving a stray blank row per fence in an editable buffer), `markdown-ts-table-fill-cells` reflows a table's data rows to a maximum column width, and `markdown-ts-hide-markup t` enables markup hiding by default. Obsidian's `[[wiki]]` links and `![[embeds]]` are not CommonMark and are not here — see `markdown-obsidian.el`. No in-Emacs preview package is configured: `markdown-preview-mode` (and classic `markdown-live-preview-mode`, and before them `grip-mode`) all dragged in `markdown-mode` plus a `web-server` recipe workaround and a major-mode-stubbing advice, just to shell out to pandoc — so all of it was removed. Preview is done from a terminal instead (`pandoc --from=gfm --to=html5 file.md -o file.html`, optionally with a watcher like `entr`/`watchexec`). Classic `markdown-mode` is no longer configured or used here at all; it is only installed if another package pulls it in as a dependency (e.g. `rustic`). Visual line wrapping is handled by `syntaxes/markdown.el` via `soft-wrap-mode`. → `docs/modules/markdown-config.md`
- `markdown-obsidian.el`: Obsidian's two non-CommonMark link forms — `[[name]]`/`[[name|alias]]` wiki links and `![[file]]`/`![[file|alias]]` embeds — rendered and followed in `markdown-ts-mode`. Loaded from the bottom of `markdown-config.el` only when `markdown-config-enable-obsidian` is non-nil (off by default, read at load time), and the dependency runs one way: it borrows `markdown-config--link-keymap` and registers on `markdown-config-follow-link-functions`, while `markdown-config.el` names nothing from here, so the option being off leaves no trace. The grammar has no `wiki_link` node, so detection is text-level: one bounded single-line regex per visible window as a font-lock keyword layered on the treesit rules, applying `markdown-obsidian-wiki-link-face` to the visible label, clickability to the whole span (brackets and an embed's `!` included, so a click lands whether or not markup is hidden), and `invisible` against the bundled `markdown-ts--markup` spec so `markdown-ts-toggle-hide-markup` toggles wiki markup with everything else. Path resolution is Obsidian's: a name containing a `/` is vault-relative and resolves from the nearest ancestor holding a `.obsidian` directory (falling back to the visited file's directory outside a vault), a bare name resolves from the visited file's directory; `.md` is appended only when the name has no extension, and nothing is ever created — a missing target signals `user-error`. Embeds render as an image under `markdown-obsidian-inline-embed-images` (and the bundled `markdown-ts-inline-images`) via **two** overlays, a one-character `display` of the image plus an `invisible` tail over the rest of the markup: an `after-string` or a wide `display` span lets `window-start` park where `pixel-scroll-precision-mode` cannot anchor it, which is the bug#64252 one-image scroll jump. An embed's alias becomes the image's `help-echo` rather than a caption line, for the same reason. → `docs/modules/markdown-obsidian.md`
...
- `gnus-config.el`: Gnus settings and the places Gnus writes. Every path is set by name — `gnus-home-directory`, `gnus-directory`, `gnus-cache-directory`, `gnus-agent-directory`, `gnus-kill-files-directory`, `gnus-article-save-directory` and `gnus-startup-file`, all under `gnus/` inside `emacs-config-state-dir`. Setting only the first two would be enough if gnus.el were still unloaded, since it computes the others from them at each defcustom; it is not, because the default `org-modules` includes `ol-gnus`, `ol-gnus` requires `gnus-sum`, and org-config.el is loaded well before this module. Derived, `newsrc.eld` therefore lands in `~` while the agent directory (defined in a file loaded later) lands correctly. `gnus-startup-file` names the base and Gnus writes `newsrc.eld` beside it, so the `.eld` is the name handed to `emacs-config-state-file`, whose LEGACY argument moves a copy left in `~` by an earlier run. `gnus-read-active-file` is t, because the default `some` asks a server only about groups already named in `newsrc.eld`, which leaves a server nothing is subscribed to yet invisible and `U` with nothing to complete — that default is for Usenet servers carrying tens of thousands of groups, and news.yhetil.org carries 20. `gnus-newsgroup-maximum-articles` and `gnus-large-newsgroup` are both 5000, which shows the newest 5000 articles of any group with no prompt: the first drops anything older when the unread list is built (`gnus-list-of-unread-articles` starts at `last - maximum + 1`), the second is the count above which Gnus asks how many to fetch, and equal values mean the question never arises — at the price of articles below that window being unreachable from the summary. Global rather than per-group, so no group name appears in this file. `gnus-agent-auto-agentize-methods` is `(nntp)`, which hands NNTP servers to the Agent so headers and articles are kept on disk rather than refetched on every group entry; Gnus reads that variable only while the agent's `lib/servers` file is absent, after which the covered servers are edited in the server buffer with `J a` and `J r`. The plain-text `.newsrc` is neither read nor written (`gnus-read-newsrc-file` / `gnus-save-newsrc-file` nil), so `newsrc.eld` alone holds the subscriptions; those are state, not configuration, which is why no module lists them and they stay out of this repo. A group newly appearing on a server is killed rather than parked as a zombie (`gnus-subscribe-newsgroup-method`), so the group buffer lists what `U` subscribed to and nothing else, and `gnus-use-full-window` is nil. No server is named here — see `gnus-servers.el`.
- `gnus-servers.el`: which servers Gnus contacts, and nothing else. `gnus-select-method` is `(nnnil "")` — no primary server — so every server is one entry in `gnus-secondary-select-methods` and adding an account later is an addition rather than a rearrangement. Currently one: `news.yhetil.org` over plain NNTP on port 119 (that host opens no TLS port), which mirrors the GNU mailing lists as newsgroups with no account and no password — `yhetil.emacs.orgmode` for emacs-orgmode@gnu.org, `yhetil.emacs.bugs` for the debbugs traffic. The server refuses posting ("post via email"), so a reply leaves as mail through `smtpmail` with `S W` rather than as a followup with `F`.
- `debbugs-config.el`: client for the GNU bug tracker at debbugs.gnu.org over its SOAP interface (package `debbugs`, which pulls `soap-client`) — a different thing from the bug mail in `yhetil.emacs.bugs`, since a report here carries a package, a severity, tags and an open/closed state, can be queried on any of them, and can be changed, with the control message to control@debbugs.gnu.org composed by the package. Entry points are `debbugs-gnu` (severities and packages), `debbugs-gnu-bugs` (given numbers), `debbugs-gnu-my-open-bugs`, `debbugs-gnu-search` and `debbugs-gnu-patches`; a report's thread opens in a Gnus ephemeral group, which is `debbugs-gnu-mail-backend`'s default. The one setting is `debbugs-gnu-persistency-file`, moved to `emacs-config-state-dir`: it holds `debbugs-gnu-local-tags` and `debbugs-gnu-local-marks`, which nothing rebuilds, and the package's default is a defvar naming `user-emacs-directory` — a symlink into this worktree. Without the package, `M-x gnus-read-ephemeral-emacs-bug-group` still fetches a report's messages by number; it is the state that needs this module.
