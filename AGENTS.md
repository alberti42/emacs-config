...
- `welcome-config.el`: startup splash buffer shown in GUI frames when Emacs is launched without file arguments. Displays a centered logo from `goodies/` and a "Welcome to Emacs!" title. Horizontal centering uses a `space :align-to (- center (0.5 . ,image))` display property so Emacs does the pixel math internally — char-unit arithmetic from `image-size` mis-centres on Retina/HiDPI. Vertical centering derives a line count from `(cdr (image-size image t))` (pixels) divided by `frame-char-height`. `q` is bound to `bury-buffer` (the buffer is kept for the session, not killed); `M-x show-welcome-buffer` resurfaces it and re-lays it out for the current window size. Wheel scrolling is suppressed via `minor-mode-overriding-map-alist` keyed on `pixel-scroll-precision-mode`: `local-set-key` is not enough because `pixel-scroll-precision-mode-map` (which carries `ultra-scroll`) binds `<wheel-up>`/`<wheel-down>` at `minor-mode-map-alist` priority, above the buffer-local map; the overriding alist sits above that and shadows per-buffer.
...
- `terminal-config.el`: terminal emulator settings — sets `xterm-256color` for `term` and `eshell`, sets `ESHELL` env var to `shell-file-name`, advises `term-handle-exit` to close the window when the process exits, and configures two terminal emulators: `vterm` (libvterm-backed) and `ghostel` (libghostty-vt-backed, ~2x faster throughput than vterm, adds Kitty keyboard protocol, OSC 8 hyperlinks, 5 underline styles, mouse passthrough, auto shell integration). Both share the same key bindings pattern: `C-@` (= C-SPC) and `C-M-m` are forwarded as raw bytes (`\C-@` and `\e\r`) using `*-send-string` instead of `*-send-key`, which re-encoded keys through the C module and produces CSI-u sequences when an app like Claude Code has enabled that mode. `C-SPC` cannot be used as the bind key — Emacs resolves it to `set-mark-command` from `global-map` before the mode map is consulted; `C-@` must be used instead. Also binds `C-g` to forward BEL so terminal apps receive it. `C-b` is exposed to Emacs in both emulators so that the `tmux-map` prefix (windows-config.el) and `C-b <arrow>` window navigation work inside terminal buffers: for vterm, `"C-b"` is added to `vterm-keymap-exceptions` via plain `setq` followed by `(define-key vterm-mode-map (kbd "C-b") nil)`. `customize-set-variable` / `setopt` / `:custom` in `:config` must NOT be used — their `:set` handler calls `vterm--exclude-keys`, which unbinds `C-c` in the map and wipes all `C-c X` bindings (`C-c C-t` copy-mode, `C-c C-l`, `C-c C-r`, `C-c C-n`, `C-c C-p`) that the `vterm-mode-map` defvar added after the exclude pass. The `:set` handler only re-runs `vterm--exclude-keys`; it does not replay the defvar, and vterm exposes no rebuild helper. `setq` bypasses the handler; the manual `define-key` performs the single excise `vterm--exclude-keys` would have done for that one key. In `vterm-mode-map`, `C-c C-c` is bound to `vterm-send-C-c` (SIGINT) because `C-c` is an Emacs prefix. For ghostel, `"C-b"` is added to `ghostel-keymap-exceptions` AND `(define-key ghostel-mode-map (kbd "C-b") nil)` is called explicitly — the `add-to-list` alone is insufficient because `ghostel-mode-map` is a `defvar` built at load time from the exceptions list, so `:config` runs too late to influence the initial build; the explicit `define-key nil` is required to remove the already-built binding. Implements `ev` editor integration: `ev-open-file` (registered in both `vterm-eval-cmds` and `ghostel-eval-cmds`) opens a file and binds `C-c C-q` to `ev-done`, which writes a semaphore file to unblock the shell-side `ev` script without involving the Emacs server.
...
- `lsp-core.el`: shared LSP configuration (`lsp-mode`, `lsp-ui`, `flycheck`, `yasnippet`). Diagnostics use flycheck (left-fringe indicators). Hover info is displayed via three independent systems: (1) **ElDoc** (echo area) — enabled via `lsp-eldoc-enable-hover t`, shows the symbol signature only (`lsp-eldoc-render-all nil`); (2) **lsp-ui-sideline** — shows hover text, diagnostics, and code-action hints inline at the right of the current line (`lsp-ui-sideline-show-hover t`); (3) **lsp-ui-doc** child frame — shown automatically at point when the cursor is on a symbol (`lsp-ui-doc-show-with-cursor t`), or on demand via `lsp-ui-doc-glance` (`C-c l h g`); in GUI uses a child frame at point, in TTY falls back to a window split. Includes a global surgical patch for `lsp--parser-on-message` to implement "Kind-First Routing" (prioritizing the `method` field over `id`), which prevents protocol deadlocks when server-initiated requests (e.g. `workspace/configuration`) collide with client IDs (e.g. completions). See `docs/lsp-mode-collision-resolution.md` for details. Configures `yasnippet` globally via `yas-global-mode`, using a custom snippet directory (`yasnippets/` in the config root). **Note**: the snippet root must NOT contain control files like `.yas-parents` or it will fail to scan subdirectories; folder names are case-sensitive and must match major-mode symbols (e.g., `LaTeX-mode` for AUCTeX). Use `yas-activate-extra-mode` to bridge naming gaps.
- `apheleia-config.el`: Formatter configuration via `apheleia` (e.g. `ruff` for Python).
- `pyenv-config.el`: per-buffer pyenv version selection, with no external package dependency. Relies on pyenv's own shim behavior as the default: shims walk up from the subprocess CWD looking for `.python-version`, and Emacs subprocesses inherit `default-directory` as CWD, so opening a file inside a project is enough for `python`/`pip`/etc. to resolve correctly — no Emacs-side logic required. On top of that default, the module adds two override paths that perform a full `pyenv activate`-equivalent buffer-locally — PYENV_VERSION, VIRTUAL_ENV, PATH (prepended with the env's `bin/` in a buffer-local `process-environment`), and buffer-local `exec-path` (same prepend). The environment's filesystem prefix is resolved once at activation via `pyenv prefix <version>`. The override paths are: (1) a dir-local variable `pyenv-version` (string, declared safe via `safe-local-variable`) applied from a `hack-local-variables-hook` — useful for project overrides you don't want to commit as a `.python-version` file, and sufficient for long-running subprocesses like coding agents that need the venv's `bin/` on their own PATH; (2) `pyenv-activate-buffer` (interactive; completing-read over `pyenv virtualenvs --bare` via `process-lines` — same list `pyenv activate <TAB>` offers, i.e. virtualenv canonical names and aliases) for ad-hoc picks. `pyenv-deactivate-buffer` drops the buffer-local `process-environment` and `exec-path` copies in one go, reverting all four changes at once. Public names use the `pyenv-` prefix (matching the underlying tool); the filename keeps the `-config.el` convention of the rest of the repo. The `pyenv-mode` package is intentionally NOT used — it would duplicate work already done by `env-config.el` (shims in `exec-path`) and would force global rather than per-buffer scope.
- `lsp-python-config.el`: Python LSP via `lsp-pyright` (configured for basedpyright). Activates in `python-mode` / `python-ts-mode` buffers AND inside org-babel Python src blocks opened with `C-c '`. For the latter, the edit-special buffer (which has no `buffer-file-name` by default and would make lsp-mode silently bail) is associated with a phantom `.py` file under `<org-dir>/._aux/org-src-<orgbase>.py`; the buffer is pre-written silently via `write-region`, cleaned up on `kill-buffer`, and opted out of file watchers. Activation for src blocks relies on **hook ordering**: `python-mode-hook` fires first and schedules `lsp-deferred`; `org-src-mode-hook` then runs `my/org-src-python-lsp-enable` which sets `buffer-file-name`/`buffer-file-truename` before the deferred callback actually starts LSP — producing a single clean `didOpen`. The activation hook is held in a local `let` closure (not a global `defun`) to avoid `M-x` pollution; trade-off is that re-evaluating the form accumulates hook entries until Emacs restart. Collisions on the phantom path (simultaneous edits of multiple blocks, leftover files from a crashed session) are resolved by `my/unique-file-path` in `utils.el`, which appends `_1`, `_2`, ....
...
- `org-config.el`: Org mode configuration. Installs Org from **tecosaur's fork** (`https://git.tecosaur.net/tec/org-mode.git`, branch `dev`) to get karthink's `org-latex-preview` with auto-preview, live updates, and dvisvgm SVG rendering — this feature is not yet merged into mainline Org. Loads `org-tempo` explicitly (not auto-loaded since Org 9.2) so `<s TAB`, `<q TAB`, etc. expand to structure blocks. Tunes `C-c '` (`org-edit-special`) with `org-src-window-setup 'current-window`, `org-src-tab-acts-natively t`, `org-src-preserve-indentation t` — these settings are load-bearing for the Python-LSP-in-org-babel workflow in `lsp-python-config.el` (the edit buffer needs to behave like a real Python buffer for LSP to attach cleanly). Sets `org-image-actual-width '(800)` as the inline-image display cap; the list form `(N)` (rather than bare `N` or `t`) enables the `#+ATTR_ORG` / `#+ATTR_HTML: :width Npx` per-image fallback mechanism. Python babel defaults are deliberately minimal — just `:results output :exports both`; matplotlib-specific setup (Agg backend, SVG savefig format, imports, `rcParams`) lives in per-file setup blocks or yasnippets (see `yasnippets/org-mode/` and `docs/pyplot-tutorial.org`). Installs `org-appear` for automatic emphasis-marker / link / sub-superscript visibility toggling based on cursor position.
- `jupyter-config.el`: emacs-jupyter for remote Jupyter kernels via the Jupyter Server protocol (HTTP/WebSockets); on-disk format stays `.org`, no `.ipynb`. Registers `jupyter` as an org-babel language so `#+begin_src jupyter-python :session http://localhost:PORT :kernel python3` blocks are recognized; the `:session URL` value routes through emacs-jupyter to a remote kernel. Includes a workaround for the `zmq` Emacs C dynamic module (required by emacs-jupyter even for server-mode workflows): the bundled `Makefile` fails to link `emacs-zmq.dylib` on recent macOS toolchains (libtool rejects the `-Wl,libzmq/src/.libs/libzmq.a` syntax), AND emacs-zmq's own auto-download misses the release asset on Apple Silicon (Emacs reports `aarch64-apple-darwinN.N` in `system-configuration` while the upstream asset is named `arm64-apple-darwinN.N` — `string-prefix-p` fails), AND straight wipes the build dir on every rebuild. The module's `jupyter-config-ensure-zmq-dylib` downloads the matching prebuilt module from upstream releases (pinned to `jupyter-config--zmq-version`, must track `zmq-emacs-version` in zmq.el) into `straight/build/zmq/`. Hooked via `with-eval-after-load 'zmq` rather than `:init` — fires synchronously inside zmq.el's `(provide 'zmq)`, after autoloads for `zmq-core` are registered but before any zmq function is called, so the dylib is in place when `zmq-load` first looks for it. This also moves the network/disk work out of Emacs startup and into first-jupyter-use.
- `jupyter-config.el`: emacs-jupyter for remote Jupyter kernels via the Jupyter Server protocol (HTTP/WebSockets); on-disk format stays `.org`, no `.ipynb`. Registers `jupyter` as an org-babel language so `#+begin_src jupyter-python :session http://localhost:PORT :kernel python3` blocks are recognized; the `:session URL` value routes through emacs-jupyter to a remote kernel. Sets `org-babel-default-header-args:jupyter-python` to `(:async yes :kernel python3 :exports both :results output)` — note this is a **separate** alist from `org-babel-default-header-args:python` (defined in `org-config.el`); `jupyter-python` is its own babel language and inherits nothing from the python defaults. Without this, jupyter-python blocks default upstream to `:results value` (Jupyter's `Out[N]:` last-expression semantics) and silently swallow `print(...)` output, which is surprising for users coming from plain `python` babel blocks. Stdout from jupyter-org executions does **not** appear in the `*jupyter-repl[…]*` buffer — that buffer only renders messages whose `parent_header.msg_id` matches the REPL client's own requests; org-babel uses a separate jupyter-org client. So `print(...)` output is captured into the org `#+RESULTS:` drawer (gated by `:results`), and the REPL buffer is for interactive REPL work. Includes a workaround for the `zmq` Emacs C dynamic module (required by emacs-jupyter even for server-mode workflows): the bundled `Makefile` fails to link `emacs-zmq.dylib` on recent macOS toolchains (libtool rejects the `-Wl,libzmq/src/.libs/libzmq.a` syntax), AND emacs-zmq's own auto-download misses the release asset on Apple Silicon (Emacs reports `aarch64-apple-darwinN.N` in `system-configuration` while the upstream asset is named `arm64-apple-darwinN.N` — `string-prefix-p` fails), AND straight wipes the build dir on every rebuild. The module's `jupyter-config-ensure-zmq-dylib` downloads the matching prebuilt module from upstream releases (pinned to `jupyter-config--zmq-version`, must track `zmq-emacs-version` in zmq.el) into `straight/build/zmq/`. Hooked via `with-eval-after-load 'zmq` rather than `:init` — fires synchronously inside zmq.el's `(provide 'zmq)`, after autoloads for `zmq-core` are registered but before any zmq function is called, so the dylib is in place when `zmq-load` first looks for it. This also moves the network/disk work out of Emacs startup and into first-jupyter-use.
...
- `windows-config.el`: window navigation, splitting, resizing, joining, reflowing, swapping, deletion, and zoom — parallels tmux pane operations. Defines `tmux-map` on `C-b` (shadowing `backward-char`) as the pane prefix, matching tmux muscle memory. `C-b <arrow>` navigates between Emacs windows and falls through to `tmux select-pane` at the frame edge when running inside tmux. `C-b C-<arrow>` resizes the active border (tmux convention: arrow direction = which way the shared border moves). `C-b S-<arrow>` joins the current window as a split adjacent to the neighbour in that direction. `C-b M-S-<arrow>` is "always-move" reflow: behaves like `S-<arrow>` when there is a neighbour, but at the frame edge repositions the window to the far side as a full-edge split, so reshaping never gets stuck. `C-b M-<arrow>` swaps buffers with an adjacent window (`windmove-swap-states-*`). `C-b %` splits right and switches to the other buffer; `C-b "` splits below. `C-b x` deletes the current window. `C-b z` toggles single-window zoom (same function as `C-x 1`: winner-based restore if already single, else `delete-other-windows`). `C-b b` forwards the prefix through to tmux so tmux-native keys (`c`, `n`, `p`, `d`) still reach it. Resize / join / reflow / swap all carry `:repeat t` keymaps so consecutive chords can be pressed without the `C-b` prefix. `C-x 2` / `C-x 3` are rebound to split and switch to the other buffer. `C-x m` jumps to the active minibuffer (overriding `compose-mail`).
- `themes-config.el`: theme loading pipeline — loads `theme-harmonize` and `zac-theme-autodetection` via `use-package` (`:straight nil`, `:load-path emacs-config-dir`), sets `theme-harmonize-tty-line-number` and `zac-load-theme-callback`, installs and configures `modus-themes`, then loads `zac-theme-autodetection` last. Explicitly sets `modus-themes-mixed-fonts nil`: when `t`, modus makes code/table faces inherit from `fixed-pitch` instead of `default` — useful only when `default` is proportional. Since `default` is already monospace (see `early-init.el`), enabling it routes org tables and src blocks through whatever `fixed-pitch` resolves to (Courier on macOS, different metrics than JetBrainsMono) and they misalign. Keep off unless `default` ever becomes proportional.
- `theme-harmonize.el`: synchronizes package faces with the active theme after every theme change. Sets `line-number` background (TTY only, via `theme-harmonize-tty-line-number`) to match the terminal emulator's padding color. Propagates the `line-number` background to git-gutter faces so the gutter column blends uniformly. Sets the background of `compilation-error`, `compilation-warning`, and `compilation-info` to match `line-number` so flymake left-margin indicators blend with the gutter column. Also propagates the `line-number` background to the `margin` face (guarded by `(facep 'margin)`) — this face is introduced by a pending Emacs patch (bug#80693) that allows customizing the margin background color; the guard makes the block a no-op on unpatched builds. Sets the `header-line` face background to match `line-number` so the lsp-mode headerline breadcrumb bar blends uniformly — the individual `lsp-headerline-breadcrumb-*` faces carry no background of their own; the bar background is entirely determined by `header-line`. Hooks into `enable-theme-functions` (Emacs 29+).
...
- emacs-jupyter (`jupyter-config.el`):
  - A reachable Jupyter Server. Local install: `pip install jupyter-server`; typical workflow is to start it on a remote host (`jupyter server --no-browser --port=8888`) and forward the port via SSH (`ssh -L 8888:localhost:8888 host`) so `:session http://localhost:8888` blocks land on the remote kernel. Auth token from `jupyter server list`.
  - Prebuilt `emacs-zmq` dynamic module from upstream releases (downloaded automatically by `jupyter-config-ensure-zmq-dylib` on first use; see the module's docstring for why this bypass exists).
