...
- `dired-config.el`: Dired customizations. Reuses a single Dired buffer when navigating (`dired-kill-when-opening-new-dired-buffer`), enables `dired-dwim-target`, moves deletions to trash. Loads `dired-x` with `dired-omit-mode` on by default (hides dotfiles via regex `\`[.][^.]`, excluding `.`/`..`), toggled with `.` (overriding the default `dired-clean-directory` binding). Installs `dired-narrow` (bound to `/`). Adds `O` for `dired-open-with` (open via OS / reveal in file manager), and `,` followed by `a/A/m/M/b/B/e/E` for Yazi-style sorting.
...
- `lsp-kotlin-config.el`: Kotlin LSP via `kotlin-ts-mode` + `kotlin-language-server` (lsp-mode built-in `lsp-kotlin`). Requires `kotlin-language-server` on `PATH` (`brew install kotlin-language-server`).
...
- Kotlin LSP (`lsp-kotlin-config.el`):
  - `kotlin-language-server` (via `brew install kotlin-language-server`)
